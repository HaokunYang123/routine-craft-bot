...
must_haves:
  truths:
    ...
    - "Callback sets role from URL on profiles where role is null (once-only, supports AUTH-10)"
    - "User is routed to correct dashboard based on profile role from database"
  artifacts:
    - path: "src/pages/AuthCallback.tsx"
      provides: "OAuth callback handler that sets role once"
      min_lines: 40
    - path: "src/App.tsx"
  ...
  key_links:
    - from: "AuthCallback.tsx"
      to: "supabase.from('profiles').update({ role })"
      via: "update where role is null"
      pattern: "update.*role.*is.*null"
    - from: "AuthCallback.tsx"
      to: "/dashboard or /app"
...
Purpose: Handle the OAuth flow completion, SET the role from URL once (profile created by trigger with role=null), and route user to correct dashboard.

Note: This plan works with 15-01's trigger which creates profiles with role=null.
The callback sets role ONCE from the URL parameter. This supports AUTH-10 (immutability):
- Role cannot be CHANGED once set
- Role CAN be SET once from null
