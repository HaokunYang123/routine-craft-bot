...
2. Find and DELETE only the useEffect block (approximately lines 82-107) that creates the 'coach-dashboard-updates' channel. This block:
   - Starts with comment `// Real-time subscription for task completions`
   - Has condition `if (!user || groups.length === 0) return;`
   - Creates `supabase.channel('coach-dashboard-updates')`
   - Has cleanup `return () => { supabase.removeChannel(channel); }`

   **Keep the loadGroupStats() function definition** (it's defined elsewhere, not inside this useEffect). React Query's automatic refetch will call it when cache is invalidated.
