...
must_haves:
  truths:
    - "User sees task checkbox update instantly when clicking (uses React Query cache)"
    - "User sees rollback if task completion fails (hook handles via onError)"
    - "StudentHome uses useAssignments mutation instead of local setTasks"
    - "StudentCalendar uses useAssignments mutation instead of local setTasks"
  artifacts:
