...
5. beforeEach:
  ...
  <verify>File exists and TypeScript compiles: `tsc --noEmit src/pages/Dashboard.test.tsx`</verify>
...
1. "displays welcome message with user name after loading":
   - Mock profile response:
     ...
     ```
   - Mock stats responses using then() for Promise.all queries (NOTE: count field is critical for `count: "exact"` queries):
     ```typescript
     const mock = getMockSupabase();
     // Each count query returns via then - the count field (not data) contains the result
