...
1. **No console.error in converted files:**
   ...
   grep -r "console.error" src/hooks src/pages src/components --include="*.ts" --include="*.tsx" | \
...
2. **handleError imported in all converted files:**
   ```bash
   grep -l "handleError" src/hooks/*.ts src/pages/*.tsx src/pages/*/*.tsx src/components/*.tsx 2>/dev/null
   ```

3. **Build succeeds:** `npm run build` with no errors

4. **Context included:** All handleError calls include component and action
</verification>
